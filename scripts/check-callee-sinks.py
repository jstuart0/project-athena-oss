...
`--scope NAME` restricts the population to a PINNED site set — a named key
in `admin/frontend/.callee-sink-phase-scopes.json` (default location;
override with `--phase-scopes`), generated by
`scripts/generate-callee-sink-phase-scope.py` before a phase's call-site
edits land. See `load_phase_scope`'s docstring for why this must be a
static manifest rather than a live re-classification. A missing manifest or
undefined scope name is exit 2 (could not run), never a silent empty scope.
...
DEFAULT_PHASE_SCOPES = DEFAULT_FRONTEND_DIR / ".callee-sink-phase-scopes.json"
...
def load_phase_scope(name: str, path: Path) -> set[str]:
    """A named, PINNED site population for `--scope` — e.g. "phase3" is the
    exact set of `file:line` call sites ATHENA-66's Phase 3 converts (the 52
    wrong-primitive sites plus the sink-associated call sites fixed
    alongside them), captured once via `scripts/generate-callee-sink-phase-
    scope.py` BEFORE those sites were edited.

    Deliberately a static manifest, not a live re-classification: `collect_
    sites()` finds a site by quoted-position interpolation regardless of its
    CURRENT escape state (D9's own design — see its docstring), so a site's
    membership here does not change when its escaping is fixed. A scope
    defined by "is this currently wrong-primitive" would go vacuously empty
    the moment the phase's own conversions land, which is exactly the
    failure this mechanism exists to avoid (rule 8).

    Raises (never silently returns empty) when the manifest or the named
    scope is missing — a missing manifest is a could-not-run condition
    (exit 2 at the caller), not an empty, trivially-satisfied scope.
    """
    if not path.is_file():
        raise FileNotFoundError(f"phase-scope manifest not found: {path}")
    data = json.loads(path.read_text(encoding="utf-8"))
    if name not in data or not data[name]:
        raise KeyError(f"scope {name!r} not defined (or empty) in {path}")
    return set(data[name])


def filter_sites_by_scope(
    sites: list[tuple[Path, int, str, str, int]], scope_keys: set[str]
) -> list[tuple[Path, int, str, str, int]]:
    return [s for s in sites if f"{rel(s[0])}:{s[1]}" in scope_keys]
...
def main() -> int:
    ...
    ap.add_argument("--scope", default=None, help="a named, pinned site population (e.g. phase3) — see load_phase_scope")
    ap.add_argument("--phase-scopes", type=Path, default=DEFAULT_PHASE_SCOPES)
    ...
    if args.scope:
        try:
            scope_keys = load_phase_scope(args.scope, args.phase_scopes)
        except (FileNotFoundError, KeyError) as exc:
            print(f"ERROR: {exc}", file=sys.stderr)
            return 2
        sites = filter_sites_by_scope(sites, scope_keys)
