...
`httpx` is pinned `~=0.28` in `src/shared/pyproject.toml`; `admin/backend/requirements.in` carries its own copy of the same pin and comment (corrected per retroactive review, M8-I2 — this section previously claimed the pin was declared "in every Python service's dependency spec"). Per PEP 440, `~=0.28` means `>=0.28, ==0.*` — any `0.x` release at or above `0.28`, not "any `0.28.x` patch release" — so this prose floor does not by itself constrain a resolver onto a specific version; a compiled, hashed lock file is what actually fixes what gets installed. **Only two images have that today**: `admin/backend`'s compiled `requirements.txt` currently resolves `httpx==0.28.1`, and `apps/jarvis-web/backend`'s `requirements.in` pins `httpx==0.28.1` directly.

Every other Python image that installs `-e src/shared` — all 23 RAG services, `gateway`, `mode_service`, `orchestrator` — inherits the `~=0.28` floor transitively through athena-shared, installed *before* that image's own `requirements.txt` (generator template and every Dockerfile: `-e /app/shared` runs, then `-r requirements.txt`). `pip install -r` does not touch an already-satisfied requirement, so the `~=0.28` install from the shared step is what actually bounds these images to `>=0.28, <1.0` on every rebuild — even though each image's own `requirements.txt` additionally declares its own looser floor (`httpx>=0.24.0` on 19 RAG services plus `gateway`/`mode_service`/`orchestrator`; `httpx>=0.25.0` on `src/rag/community_events`, `src/rag/price_compare`, and `src/rag/site_scraper`). None of these 26 images has a compiled, hashed lock for httpx specifically, so the exact `0.28.x`/`0.29.x`/etc. patch actually installed floats to whatever's newest on PyPI within that range at each rebuild — not locked until a future phase (Phase 8) converts them to compiled, hashed locks the way `admin/backend` and `jarvis-web` already are. `apps/jarvis-web/backend` and `apps/chat-embed` are both `NO_SHARED_DIRS` exemptions (`scripts/lock-requirements.sh`) — neither installs `-e src/shared`, so neither uses `_build_pinned_transport` or is bound by the SSRF-guard contract below, and each floats its own httpx spec with no `~=0.28` floor at all: `jarvis-web/backend` pins `httpx==0.28.1` anyway; `chat-embed` (`httpx>=0.25.0`, no upper bound, no shared install) is the one image genuinely unbounded below `0.28`. `_build_pinned_transport` in `src/shared/url_safety.py` — the SSRF guard's IP-pinning connection layer — relies on `httpx.AsyncHTTPTransport._pool`, a private attribute of `httpx ≥ 0.28`. A version bump that removes or restructures `_pool` silently degrades the SNI/IP-pinning protection (there is a runtime feature-detection guard, but it fails open to an unpinned transport rather than failing the request). **Before upgrading httpx past `0.28.x` anywhere in this repo**, re-verify `_build_pinned_transport` against the new version — see the version-requirement notes at the top of `url_safety.py` and the tests that exercise the real (unmocked) pinned transport: `TestPinnedNetworkBackend`, `TestSniPreservation`, `TestBuildPinnedTransportFallback`.
