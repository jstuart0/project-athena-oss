...
- **Fixed (Medium)**: the prior seven-image `pkg_resources` verification ran only `import main`, which never touches `src/shared/content_fetcher.py`'s lazily-imported, `except ImportError`-guarded optional dependencies (`trafilatura`, `extruct`, `pandas`, `playwright`) — a missing `pkg_resources` there would silently flip a `HAS_*` flag to `False` without failing the smoke test. `athena-orchestrator` and `athena-rag-sitescraper` (the two images that ship those packages) were rebuilt and checked with an unguarded import of all of them plus an assertion that every applicable `shared.content_fetcher.HAS_*` flag reads `True` (see the Phase 6 entry above, corrected in place). Also corrected: the vulnerable-setuptools ceiling is `<83.0.0` (CVE-2026-59890's actual fix floor), not `<81`.
...
- **Corrected (Medium)**: `CONTRIBUTING.md`'s httpx section overstated what the `~=0.28` prose pin guarantees. Corrected to state plainly that only `admin/backend`'s compiled lock (currently resolving `httpx==0.28.1`) and `apps/jarvis-web/backend`'s explicit `httpx==0.28.1` pin hold httpx at that version today; every other Python image that touches httpx (all 23 RAG services, `gateway`, `mode_service`, `orchestrator`, `apps/chat-embed`) still carries a hand-written, uncompiled `requirements.txt` with only a loose, unlocked floor (`httpx>=0.24.0`, no upper bound, for the first group; `httpx>=0.25.0` for `chat-embed`) and floats to whatever's newest on PyPI at each rebuild until a future phase locks them. Also fixed `chat-embed` being called "hand-pinned" when its spec floats rather than pins.
