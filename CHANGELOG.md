...
- **Verified (R13 mitigation)**: enumerated internal outbound POST callers of admin-backend (`src/shared/admin_config.py`, `src/orchestrator/main.py`) ahead of the FastAPI bump — all use `json=` or target endpoints with no JSON body parameter, so FastAPI 0.132's mandatory `Content-Type: application/json` cannot break an internal caller.
