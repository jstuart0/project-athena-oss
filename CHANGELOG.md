...
> **Ticket:** ATHENA-63
> **Commits:** `1657900` (Phase 1), `8bd26fd` (Phase 2), `8fb9b7e` (Phase 3), `689fd11` (Phase 4), `93b2643` (Phase 5), `8c33829` (Phase 5 follow-up), `6814b6f`, `d9ff720`, `c84672c`, `e11e244`, `3e359ee`, `4df47b8` (Phase 6), `39daf6f`, `449bbfe`, `7b07a31` (Phase 7), `ccccd47` (Phase 8), `21a4b45`, `b3b9a9e`, `32df611` (Phase 9), `9f0dacd` (merge into the campaign branch)
...
- **Security fix**: `apps/jarvis-web/backend/requirements.{in,txt}` — `python-multipart` `0.0.20` → `>=0.0.32` (locked at `0.0.32`), clearing seven advisories affecting `0.0.20`: CVE-2026-24486 (arbitrary file write, fixed 0.0.22), CVE-2026-40347 (0.0.26), CVE-2026-42561 (0.0.27), CVE-2026-53537 (0.0.30), CVE-2026-53538 (0.0.30), CVE-2026-53539 (0.0.30), CVE-2026-53540 (0.0.31). CVE-2024-53981 (the CVSS 8.7 advisory that blocked a predecessor campaign) was already patched at `0.0.20` (fixed in `0.0.18`) and is not one of the seven cleared here.
...
- **Fixed**: deleted `src/shared/requirements.txt` — a duplicate of `src/shared/pyproject.toml`'s `dependencies` array with zero consumers anywhere in the tree outside this campaign's own planning documents. `pyproject.toml` is now the sole source of shared's dependency spec.
...
- **Disclosed**: the resolved lock carries `starlette==0.52.1`, which still has 5 distinct CVEs — CVE-2026-48710, CVE-2026-48817, CVE-2026-48818 (Windows-only, not applicable to this deployment), CVE-2026-54282, CVE-2026-54283 — with fix floors `1.0.1`–`1.3.1` that require starlette's `1.x` major, delivered in Phase 7. This is a strict reduction from `main`'s pre-campaign `starlette==0.27.0` (7 distinct CVEs: the above 5 plus CVE-2024-47874 and CVE-2025-54121, both already fixed at 0.52.1) — Phase 4 introduced no new advisory. `admin/backend/app/routes/sms_webhook.py:48,56` reconstructs `str(request.url)` for Twilio HMAC verification; `main.py` registers no `TrustedHostMiddleware`.
- **Known limitation**: `test_success_path_not_artificially_delayed` (`admin/backend/tests/test_security_hardening.py`) is a wall-clock assertion (`elapsed_ms < 1500`) that fails only when the test suite runs under amd64 emulation on non-x86 hardware (measured 4.1–4.8 s under QEMU, ~67× slower than native); it passes natively on arm64. Not introduced by this campaign; the admin-backend suite is otherwise clean across Phases 4, 6, and 7 (0 errors, 0 unexplained failures).
...
- **Known limitation**: the four hand-written Dockerfiles' `--no-deps`/`--no-build-isolation` line and the 29 `.in`-to-`.txt` lock syncs have no CI gate — enforcement today is local (`make lock-check` and the local per-phase image builds this campaign ran by hand). The lock content itself is still consumed on every image build regardless, and the 23 generated RAG Dockerfiles keep their drift gate (the generator `--check`). Tracked as **[ATHENA-81](https://plane.xmojo.net)**.
