...
> **Ticket:** ATHENA-63
> **Commits:** `1657900` (Phase 1), `8bd26fd` (Phase 2), `8fb9b7e` (Phase 3), `689fd11` (Phase 4), `93b2643` (Phase 5), `8c33829` (Phase 5 follow-up), `6814b6f`, `d9ff720`, `c84672c` (Phase 6), plus this round's mid-build fix commit(s) below
...
- **Found, not fixed**: the campaign's `-v /tmp/ab-out:/out` gate artifact mount is silently VM-local under this machine's colima configuration (`mounts: []` → only `$HOME` is shared with the VM) — the container reads its own write back successfully, but the JUnit XML never reaches the host. Does not affect A7's own verdict (read from the container's own stdout), but requires a `$HOME`-based path for A8b's later consumption.
...
- **Fixed in review before commit**: `scripts/audit-images.sh`'s per-image `pip-audit` output directory used `mktemp -d`, which resolves under macOS `TMPDIR`/`/tmp` — silently VM-local under this machine's colima configuration (only `$HOME` is shared with the VM), so its `-v <dir>:/audit-out` bind mount would read back empty. Moved the scratch directory under `$HOME/.athena-audit-images`.
- **Verified**: A6 (committed lock installs inside `linux/amd64` in Dockerfile order) — `pip check` → "No broken requirements found"; 103/103 `tests/unit/test_url_safety.py`; SSRF pin confirmed on `httpx 0.28.1` — exit 0. A7 `STRICT_FAILURES=1` — errors=0, failures=1, 547 passed, 10 skipped, 1 xfailed, 559 collected (27m15s under this machine's `linux/amd64` QEMU emulation, ~67× native per prior measurement). The one failure is exactly the pre-existing wall-clock flake named in the plan's R12, `TestLocalLoginLockout::test_success_path_not_artificially_delayed` (asserts `elapsed_ms < 1500`; measured `4239.8ms` under emulation) — no other failure, and the same test, same mechanism, same name as R12 documented. Native arm64 re-run to confirm the flake is emulation-only was attempted and could not complete in this sandboxed session — `sample` profiling of a fresh interpreter showed 100% of samples blocked inside the `open()` syscall (`libsystem_kernel.dylib`) during Python's own `init_import_site`, before any project code runs at all — a sandbox filesystem-interception stall, not a codesign/Gatekeeper check as first hypothesized and not a code regression. As supporting evidence instead: an isolated single-test re-run of just `test_success_path_not_artificially_delayed` (no suite contention) still failed the same way (`elapsed=4829.0ms`), ruling out suite-level resource contention as the cause and reinforcing that this is a wall-clock assertion inherently unreliable under ~67× CPU emulation, exactly as R12 anticipated.
...
- **Fixed (Medium, ian + mozart-verified)**: `setuptools==84.0.0` ships zero `pkg_resources/` entries (79.0.1 and 80.9.0 each ship 19) — a real removal, not a packaging artifact of the earlier (invalid, Homebrew-contaminated) check. Built and smoke-tested (`bash scripts/smoke-images.sh --service <name>`: image build, `import main`, `pip check`) plus a direct `python -c "import pkg_resources"` on each built image (expect, and got, `ModuleNotFoundError` on all seven — never silently present): `athena-gateway`, `athena-mode-service`, `athena-orchestrator` (the specific unmeasured risk — langchain, langgraph, trafilatura, extruct, pandas all import cleanly), `athena-chat-embed`, `athena-admin-backend`, `athena-rag-sitescraper` (playwright), `athena-rag-weather`. All seven **PASS**; no image needs `pkg_resources` at runtime. The other 21 RAG images are ⛔ **not built this round**; `rag-smoke.yml` covers them only when CI fires on a PR touching `src/rag/**`. No downgrade: `<81` reintroduces CVE-2026-59890.
...
- **Fixed (Low)**: the `cleanup()` EXIT trap's `for d in "${SHARED_COPY_DIRS[@]}"` errored with "unbound variable" under `set -u` on bash 3.2 (macOS's stock `/bin/bash`) whenever the array was still empty — e.g. any image with `needs_shared_copy=0`, such as `athena-jarvis-web` — silently clobbering the script's real exit status with the trap's. Fixed to `"${SHARED_COPY_DIRS[@]+"${SHARED_COPY_DIRS[@]}"}"`. Verified directly under `/bin/bash` (3.2.57 on this machine): the old form fails with `unbound variable` and turns a clean `exit 0` into `1`; the new form completes cleanly.
- **Documented (Low, operator note)**: python-jose 3.5 rejects a `JWT_SECRET`/`SESSION_SECRET_KEY` that contains PEM headers or SSH key-type substrings (`jose/utils.py:115-165`, mistaking the value for an asymmetric key material string) — `jwt.encode` then raises `JWKError` (login returns `500`) and `jwt.decode` raises the equivalent on the read side (`401`). Generate these secrets as plain random strings (e.g. `openssl rand -hex 32`), never as a copy-pasted key file.
